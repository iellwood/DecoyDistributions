...
Changes to their code:
    ...
    [1] Modified the scaling terms to ensure a global constant Jacobian

    See the lines of code:
        l_fixed = tf.get_variable(name='constant_l', shape = s, dtype=tf.float32, initializer=tf.initializers.zeros())
        l_fixed = l_fixed*(1 - b)
        l = l_fixed + l - (1 - b)*tf.reduce_sum(l, [1, 2, 3], keep_dims=True)/tf.reduce_sum((1 - b), [1, 2, 3], keep_dims=True)


    [2] The evaluation of a sample in the prior, compute_log_density_x(), has been modified to accomodate our special seed
    distribution.

Note that the FlowGAN model must always be run first "backwards" on an image, before it
...
from  DecoyDistributionModel.seed import log_likelihood_of_uniform_distribution_of_log_likelihood_prior as ll
...
# def my_scale(x):
#     s = tf.cast(x > 0, tf.float32)
#     return s * (x + 1) + (1 - s) * 1 / (tf.abs(x) + 1)
#
# def log_my_scale(x):
#     return tf.log(my_scale(x))
...
class Layer():
    ...
    def backward_and_jacobian(self, y, z):
        raise NotImplementedError(str(type(self)))


def batch_norm_2(input_, name, train=True, epsilon=1e-6, decay=.1, axes=[0, 1], reuse=None, bn_lag=0., dim=[], scaling=True):
    return input_


def batch_norm(input_, name, train=True, epsilon=1e-6, decay=.1, axes=[0, 1], reuse=None, bn_lag=0., dim=[], scaling=True):
    ...
    with tf.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
        ...
        #print(tf.get_default_graph().get_name_scope(), 'dim =', dim)
        # if reuse:
        #     scope.reuse_variables()
        var = tf.get_variable("var", dim, tf.float32, tf.constant_initializer(1.), trainable=False)
        mean = tf.get_variable("mean", dim, tf.float32, tf.constant_initializer(0.), trainable=False)
    ...
    if train:
        ...
        if scaling:
            used_mean, used_var = tf.stop_gradient(mean), tf.stop_gradient(var)
        else:
...
def get_weight(name, weights_shape, init_type):
    if init_type == "uniform":
        weights = tf.get_variable(name, weights_shape, tf.float32, tf.random_uniform_initializer(-0.01, 0.01, seed=0))
    elif init_type == "normal":
        weights = tf.get_variable(name, weights_shape, tf.float32, tf.truncated_normal_initializer(stddev=0.02, seed=0))
    elif init_type == "orthogonal":
        weights = tf.get_variable(name, weights_shape, tf.float32, tf.orthogonal_initializer(seed=0))
    elif init_type == "xavier":
        weights = tf.get_variable(name, weights_shape, tf.float32, tf.contrib.layers.xavier_initializer(seed=0))
    return weights
...
class NICECouplingLayer(Layer):
    ...
    def function_l_m(self, x, name='function_l_m', reuse=False, train=False):
        with tf.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            y = nlin(tf.matmul(y, weights) + bias)
    ...
    def backward_and_jacobian(self, y, sum_log_det_jacobians, z, reuse, train):
        # tf.set_random_seed(self.seed)
        # np.random.seed(self.seed)
        with tf.variable_scope(self.name, reuse=reuse):
...
class NICEScaling(Layer):
    ...
    def backward_and_jacobian(self, y, sum_log_det_jacobians, z, reuse, train=None):
        # tf.set_random_seed(0)
        # np.random.seed(0)
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            return x, sum_log_det_jacobians - tf.reduce_sum(scale_factor), z

def simple_batch_norm_2(x):
    return x
...
class CouplingLayer(Layer):
    ...
    def function_l_m(self, x, mask, name='function_l_m', reuse=False, train=False):
        with tf.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            if not self.scaling:
                ...
                y = batch_norm(input_=y, dim=input_channel, name="g_bn_in", train=train, epsilon=1e-4, axes=[0, 1, 2], reuse=reuse, scaling=False)
            ...
            y = nlin(y)
            if self.scaling:
                y = batch_norm(input_=y, dim=channel, name="g_bn_in2", train=train, epsilon=1e-4, axes=[0, 1, 2], reuse=reuse)
            ...
            for r in range(num_residual_blocks):
                ...
                y = nlin(y)
                if self.scaling:
                    y = batch_norm(input_=y, dim=channel, name="g_bn%d_1" % r, train=train, epsilon=1e-4, axes=[0, 1, 2], reuse=reuse, scaling=False)
                ...
                y = nlin(y)
                if self.scaling:
                    y = batch_norm(input_=y, dim=channel, name="g_bn%d_2" % r, train=train, epsilon=1e-4, axes=[0, 1, 2], reuse=reuse)
                skip = y
            ...
            if 'checkerboard' in self.mask_type:
                scale_factor = tf.get_variable("g_weights_tanh_scale", [1], tf.float32, tf.constant_initializer(0.),
                                               regularizer=tf.contrib.layers.l2_regularizer(5e-5))
            else:
                scale_factor = tf.get_variable("g_weights_tanh_scale", [1], tf.float32, tf.constant_initializer(1.))

            scale_shift = tf.get_variable("g_weights_scale_shift", [1], tf.float32, tf.constant_initializer(0.))
    ...
    def forward_and_jacobian(self, x, sum_log_det_jacobians, z, reuse=False, train=False):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            y = x1 + tf.multiply(-b + 1.0, x * tf.check_numerics(tf.exp(l), "exp has NaN in forward pass scale") + m)
            log_det_jacobian = tf.reduce_sum(l, [1, 2, 3])
            sum_log_det_jacobians += log_det_jacobian

            return y, sum_log_det_jacobians, z

    def backward_and_jacobian(self, y, sum_log_det_jacobians, z, reuse=False, train=False):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ys = int_shape(y)
            ...
            l = tf.check_numerics(l, "l has NaN in backward pass")
            ...
            l_fixed = tf.check_numerics(l_fixed, "l_fixed has NaN in backward pass")


            l = l_fixed + l - (1 - b) * tf.reduce_sum(l, [1, 2, 3], keep_dims=True) / tf.reduce_sum((1 - b), [1, 2, 3], keep_dims=True)

            l = tf.check_numerics(l, "l has NaN in backward pass after combining with l_fixed")

            s = tf.check_numerics(my_scale(l), "exp has NaN in backward pass scale")
            x = y1 + (y - m)*(1.0 - b)/s
            log_det_jacobian = tf.reduce_sum(-log_my_scale(l), [1, 2, 3])
            sum_log_det_jacobians += log_det_jacobian
            return x, sum_log_det_jacobians, z
...
def compute_log_density_x(z, sum_log_det_jacobians, prior_info):
    zs = int_shape(z)
    if len(zs) == 4:
        n = zs[1] * zs[2] * zs[3]  # dimension of the Gaussian distribution
        z = tf.reshape(z, (-1, n))
    else:
        n = zs[1]
        
    if prior_info['prior_type'] == 'gaussian':
        log_density_z = -0.5 * tf.reduce_sum(tf.square(z), [1]) - 0.5 * n * np.log(2 * np.pi)
    elif prior_info['prior_type'] == 'uniform_distribution_of_log_likelihoods':
        log_density_z = ll(z, prior_info['W'])
